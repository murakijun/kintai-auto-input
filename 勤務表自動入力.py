...
def resolve_excel_path(cfg: configparser.ConfigParser) -> str:
    """
    Excelファイルパスを決定する優先順位:
      1. config.ini の [excel] path
      2. ファイル選択ダイアログ（→ 選択結果を config.ini に保存）
    """
    path = cfg.get("excel", "path", fallback="").strip()

    if path and os.path.exists(path):
        return path

    if path and not os.path.exists(path):
        print(f"[警告] config.ini に記録されたパスが見つかりません:\n  {path}")
        print("  ファイル選択ダイアログを開きます...\n")

    print("勤務表Excelファイルを選択してください（ダイアログが開きます）")
    ...
    # 次回から自動で使えるよう config.ini に保存
    cfg.set("excel", "path", selected)
    save_config(cfg)
    print(f"[情報] 選択したパスを config.ini に保存しました。\n  {selected}\n")
...
def main():
    # --- 設定読み込み ---
    cfg          = load_config()
    year         = cfg.getint("target", "year",         fallback=2026)
    month        = cfg.getint("target", "month",        fallback=4)
